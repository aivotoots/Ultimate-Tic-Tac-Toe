...
class defining_cell:
	...
	def compute_cell( self,temp_board, pos_x, pos_y):
		temp1 = pos_x/3
		temp2 = pos_y/3
		x = 3 * temp1
		y = 3 * temp2
		return self.evaluation_function( x, y, temp_board)

	def evaluation_function(self, x, y, temp_board):
		
		utility = 0
		evaluate_utility_inst = evaluate_utility()
		...
		for m in range( x, x+3):
			...
			for n in range( y, y+3):
				if temp_board[m][n] == 'x':
					numof_x += 1;
				elif temp_board[m][n] == 'o':
					numof_o += 1; 
			utility = utility + evaluate_utility_inst.utilityFunc(numof_x,numof_o);
	...
	def main_utility( self,board_state):
		...
		# for diagonal
		for i in range(1,4):
			if board_state[ 2 * i] == 'x':
				main_numof_x += 1;
			if board_state[ 2 * i] == 'o':
				main_numof_o += 1;
		main_utility = main_utility + evaluate_utility_inst.utilityFunc( main_numof_x, main_numof_o);

		main_numof_x = 0
		main_numof_o = 0
		for i in range( 0,3):
			if board_state[ 4 * i] == 'x':
				main_numof_x += 1;
			if board_state[ 4 * i] == 'o':
				main_numof_o += 1;
		main_utility = main_utility + evaluate_utility_inst.utilityFunc( main_numof_x, main_numof_o);
		...
		for i in range( 0, 3):
			...
			main_numof_o = 0
			for j in range( 0, 3):
				if board_state[ 3*i + j] == 'x':
					main_numof_x += 1;
				if board_state[ 3*i + j] == 'o':
					main_numof_o += 1;
			main_utility = main_utility + evaluate_utility_inst.utilityFunc( main_numof_x, main_numof_o);
		...
		for i in range( 0, 3):
			...
			main_numof_o = 0
			for j in range( 0, 3):
				if board_state[ 3*j + i] == 'x':
					main_numof_x += 1;
				if board_state[ 3*j + i] == 'o':
					main_numof_o += 1;
			main_utility = main_utility + evaluate_utility_inst.utilityFunc( main_numof_x, main_numof_o);		
...
class Player44:
	...
	def blocks_possible(self,temp_board,temp_block,old_move,flag):
#		while(1):
#			pass
	...
	def minimax(self,temp_board,temp_block,alpha,beta,flag,depth,old_move):
		#print "**"
		#moved = []
		#player = Player1()
		cell = defining_cell()
		if(depth == 0):
			#print cell.main_utility(temp_block)
			return cell.main_utility(temp_block) + cell.compute_cell(temp_board,old_move[0],old_move[1])

		if flag == 'x' :
			utility = -10000
			cells = self.blocks_possible(temp_board,temp_block,old_move,flag)
			if len(cells) == 0 :
				#return cell.compute_cell(temp_board,old_move[0],old_move[1])
				return cell.main_utility(temp_block) + cell.compute_cell(temp_board,old_move[0],old_move[1])
			for cell in cells:
				temporary_board = []
				for i in range(0,9):
					row = ['-']*9
					temporary_board.append(row)
	
				temporary_block_stat = ['-']*9
				
				for i in range(0,9):
					for j in range(0,9):
						temporary_board[i][j] = temp_board[i][j]

				for i in range(0,9):
					temporary_block_stat[i] = temp_block[i]

				update_lists(temporary_board,temporary_block_stat,cell,flag)
				temp = alpha
				alpha = max(alpha,self.minimax(temporary_board,temporary_block_stat,alpha,beta,'o',depth-1,cell))
				#utility = max(utility,temp)
			...
			for cell in cells:
				...
				update_lists(temporary_board,temporary_block_stat,cell,flag)
				temp = beta
				beta = min(beta,self.minimax(temporary_board,temporary_block_stat,alpha,beta,'x',depth-1,cell))
				#utility = min(utility,temp)
	...
	def move(self,temp_board,temp_block,old_move,flag):
		...
		#beta = 1000
		#print "@"
		util = self.minimax(temp_board,temp_block,-10000,10000,flag,4,old_move)
		#print util[1]
		print self.moved
		return self.moved
